...
void FilterData::updateFilter(const int filterType)
{
    switch (filterType)
    {
    case 0:
        iFilter.setType(juce::dsp::StateVariableTPTFilterType::lowpass);
        break;

    case 1:
        iFilter.setType(juce::dsp::StateVariableTPTFilterType::bandpass);
        break;

    case 2:
        iFilter.setType(juce::dsp::StateVariableTPTFilterType::highpass);
        break;
    }
}
...
void SubtractiveSynthesisAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    objFilter.iFilter.reset();
    objFilter.iFilter.prepare(spec);
...
void SubtractiveSynthesisAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    //********************************************************************************************//
    // implement the actual change in note velocity: for every message we are going to create a new
    ...
    objFilter.iFilter.setCutoffFrequency(cutoff);
    objFilter.updateFilter(filterType);
    objFilter.iFilter.process(juce::dsp::ProcessContextReplacing<float>(audioBlock));
}
